...
const PerformHook = () => {
  ...
  //마우스 포커스를 특정 입력욧에 맞추어준다.
  useEffect(() => {
    numberTag.current.focus();
  }, []);

  //평균값 변수 선언 및 평균값 처리함수 호출하기
  // useMomo훅을 사용하지 않고 호출하는 공통연산함수
  //const avg = getAverage(list);

  //useMemo훅을 사용해 호울하는 공통연산함수
  //useMemo(실행할함수,[데이터변경조건상태값지정])
  //list 숫자배열의 값이 변경될때만 getAverage()함수를 호출하게 한다.
  ...
  //useMemo는 특정 재사용가능함수를 특정조건에서 실행할 수 있게하여 리액트앱 성능을 담보시켜준다.
  ...
  //리엑트 성능이 개선되고 useCallback훅을 통해 관련기능을 추가 구현합니다.
  //최초로 화면 컴포넌트가 로딩이 완료될때 한번만 해당 함수가 생성됩니다.
  //useCallback(실행함수정의, [실행조건]); //실행조건이 빈배열이면 최초 컴포넌트가 로딩될때 1번만 실행함수가 만들어집니다.
