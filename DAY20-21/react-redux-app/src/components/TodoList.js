...
//리덕스 폴더안에 액션통합모듈을 참조하고 관련 액션함수(addTotoCount)를 참조합니다.
...
const TodoList = () => {
  ...
  const hanleAddtTodo = () => {
    ...
    //globalDispatch(액션합수호출);
    globalDispatch(addTodoCount(todoList.length + 1));
  };

  return (
    <div>
      할일: <input name='title' value={todo.title} onChange={handleTodo} />
      <br />
      우선순위:
      <input name='orderby' value={todo.orderby} onChange={handleTodo} />
      <br />
      <button onClick={hanleAddtTodo}>추가</button>
      <hr></hr>
      <ul>
